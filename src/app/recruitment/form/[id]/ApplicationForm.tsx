...
const ApplicationForm = ({ position }: Props): JSX.Element => {
  ...
  const handleApplicationSubmit = async (e: React.FormEvent) => {
    ...
    formData.append("address", applicationFormData.address);
    formData.append("age", applicationFormData.age);
    formData.append("category", position.title);
    formData.append("experience", applicationFormData.experience);
    formData.append("motivation", applicationFormData.motivation);
    formData.append("portfolio", applicationFormData.portfolio);
  
    if (applicationFormData.resume) {
      formData.append("resume", applicationFormData.resume);
    }
  
    if (applicationFormData.coverLetter) {
      formData.append("coverLetter", applicationFormData.coverLetter);
    }
  
    const res = await fetch('https://rt-solutions.co.jp/api/recruitment_db.php', {
      method: 'POST',
      body: formData,
    });
      ...
      <div className="max-w-4xl mx-auto px-4">
          ...
          <form onSubmit={handleApplicationSubmit} className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors hover-lift-enhanced text-gray-700"
                  placeholder="山田 太郎"
                />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea
                ...
                onChange={handleApplicationInputChange}
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors resize-none hover-lift-enhanced text-gray-700"
        ...
        <div className="text-center mt-12 stagger-fade-in stagger-delay-2">
          <Link href={`/recruitment/${position.id}`} key={position.id}>
            <button className="btn-primary text-white px-8 py-3 rounded-full font-semibold hover-lift-enhanced m-2">
              募集要項に戻る
            </button>
          </Link>
          <Link href="/recruitment">
            <button className="bg-white text-blue-600 border-2 border-blue-600 px-8 py-3 rounded-full font-semibold hover:bg-blue-50 transition-colors m-2 hover-lift-enhanced">
              採用情報一覧に戻る
            </button>
          </Link>
          <Link href="/">
            <button className="bg-gray-200 text-gray-700 px-8 py-3 rounded-full font-semibold hover:bg-gray-300 transition-colors m-2 hover-lift-enhanced">
              ホームに戻る
            </button>
          </Link>
        </div>
